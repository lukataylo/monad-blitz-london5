...
export const walkPoolAbi = parseAbi([
    "function createChallenge(uint256 stake, uint64 duration, string title, string name) payable returns (uint256 id)",
    ...
    "function getChallenge(uint256 id) view returns (address creator, uint256 stake, uint64 endTime, bool settled, uint256 pot, uint256 participantCount, string title)",
    "function getParticipants(uint256 id) view returns (address[] addrs, uint256[] steps, uint256[] payouts, string[] names)",
    "event ChallengeCreated(uint256 indexed id, address indexed creator, uint256 stake, uint64 endTime, string title)",
